...
for ii = 1:qq
    ...
    [algo_time(ii), itrs(ii), mse_err(ii)] = understanding_mle(theta_org(ii,:),theta_0);
    kk = kk+1;
    save('kk','kk');
end
...
for ii = 1:qq
    theta_0 = randi(rad*2,1,2);
    theta_0 = theta_0 - repmat([rad rad],1,1);
    [algo_time_rnd(ii), itrs_rnd(ii), mse_err_rnd(ii)] = understanding_mle(theta_org(ii,:),theta_0);
    kk = kk+1;
    save('kk','kk');
end
